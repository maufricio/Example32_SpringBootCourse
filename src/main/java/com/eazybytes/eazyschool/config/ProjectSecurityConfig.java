...
public class ProjectSecurityConfig {
    ...
    SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http, HandlerMappingIntrospector introspector) throws Exception {
        ...
        http.csrf((csrf) -> csrf.ignoringRequestMatchers(mvcMatcherBuilder.pattern("/saveMsg"))
                ...
                .authorizeHttpRequests((requests) -> requests.requestMatchers(mvcMatcherBuilder.pattern("/dashboard")).authenticated()
                        .requestMatchers(mvcMatcherBuilder.pattern("/displayMessages")).hasRole("ADMIN") //Don't put the ROLE_ prefix here
                        .requestMatchers(mvcMatcherBuilder.pattern("/closeMsg/**")).hasRole("ADMIN") //The ** means that it will match any path that starts with /closeMsg/
    ...
    public InMemoryUserDetailsManager userDetailsService() {
        ...
        UserDetails admin = User.withDefaultPasswordEncoder()
                .username("admin")
                .password("54321")
                .roles("ADMIN") //Spring will add a ROLE_ prefix. So the effective role name will be ROLE_ADMIN
                .build();
